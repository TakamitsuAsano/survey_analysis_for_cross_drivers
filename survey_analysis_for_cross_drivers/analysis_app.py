...
# 分析手法と観点
1. **クロス集計**
   - 年代、性別、居住年数などの基本属性と、満足度や意識設問との掛け合わせで、各意識・満足度のTop2（肯定派）の割合の差分が15%以上見られた顕著な組み合わせを網羅的に抽出してください。
   - さらに、Bottom2（否定派）の割合の差分が15%以上見られる顕著な組み合わせも網羅的に抽出してください。
   - **⚠️重要: トップ数個に絞るのではなく、上記の条件（15%以上の差）を満たす組み合わせは可能な限りすべて列挙してください。**

2. **ドライバー分析（ロジスティック回帰）**
   - 目的変数：「状態」や「体感・満足度」「推奨意向」を測る指標や「生活習慣・行動」への影響などの重要指標（もしデータにあれば。なければ「全体的な満足度」やポジティブな回答）
   - 説明変数：満足度や意識に関する設問
   - どの地域要素がプラスの影響（オッズ比が1.0より大きく、高いもの）を与えているかをランキング化してください。
   - 同時に、オッズ比が0.8以下のマイナス（ネガティブ）の影響を与えている要素も分けて列挙してください。
   - **⚠️重要: トップ数個に絞るのではなく、統計的に意味のある影響を与えている要素は可能な限りすべて列挙してください。**
...
2. **アプリ連携用設定データ（JSON形式）**
   - **最後に必ず**、以下のJSON形式で、各分析ごとに設定パターンを抽出してください。
   - **⚠️重要: 「クロス集計」と「ドライバー分析」については、分析サマリーで列挙したすべての組み合わせ・パターンを網羅してJSONリストに含めてください（3つに限定せず、見つかった数だけ出力してください）。**
   - 決定木とクラスター分析については3つ以上の設定パターンを出力してください。
